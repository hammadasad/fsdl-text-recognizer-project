...
class CharacterModel(Model):
    ...
    def predict_on_image(self, image: np.ndarray) -> Tuple[str, float]:
        ...
        raw_prediction = self.network.predict(np.expand_dims(image,0), batch_size=1).flatten()
        index = np.argmax(raw_prediction)
        confidence_of_prediction = raw_prediction[index]
        predicted_character = self.data.mapping[index]
